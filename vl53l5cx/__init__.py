...
class VL53L5CXResultsData:
    def __init__(self, nb_target_per_zone: int) -> None:
        ...
        # Ambient noise in kcps/spads - originally # ifndef api.VL53L5CX_DISABLE_AMBIENT_PER_SPAD
        self.ambient_per_spad = [0] * api.VL53L5CX_RESOLUTION_8X8

        # Number of valid target detected for 1 zone - originally # ifndef api.VL53L5CX_DISABLE_NB_TARGET_DETECTED
        self.nb_target_detected = [0] * api.VL53L5CX_RESOLUTION_8X8

        # Number of spads enabled for this ranging - originally # ifndef api.VL53L5CX_DISABLE_NB_SPADS_ENABLED
        self.nb_spads_enabled = [0] * api.VL53L5CX_RESOLUTION_8X8

        # Signal returned to the sensor in kcps/spads - originally # ifndef api.VL53L5CX_DISABLE_SIGNAL_PER_SPAD
        self.signal_per_spad = [0] * (api.VL53L5CX_RESOLUTION_8X8 * nb_target_per_zone)

        # Sigma of the current distance in mm - originally # ifndef api.VL53L5CX_DISABLE_RANGE_SIGMA_MM
        self.range_sigma_mm = [0] * (api.VL53L5CX_RESOLUTION_8X8 * nb_target_per_zone)

        # Measured distance in mm - originally # ifndef api.VL53L5CX_DISABLE_DISTANCE_MM
        self.distance_mm = [0] * (api.VL53L5CX_RESOLUTION_8X8 * nb_target_per_zone)

        # Estimated reflectance in percent - originally # ifndef api.VL53L5CX_DISABLE_REFLECTANCE_PERCENT
        self.reflectance = [0] * (api.VL53L5CX_RESOLUTION_8X8 * nb_target_per_zone)

        # Status indicating the measurement validity (5 & 9 means ranging OK) - originally # ifndef api.VL53L5CX_DISABLE_TARGET_STATUS
        self.target_status = [0] * (api.VL53L5CX_RESOLUTION_8X8 * nb_target_per_zone)

        # Motion detector results - originally # ifndef api.VL53L5CX_DISABLE_MOTION_INDICATOR
    ...
    def update_motion_indicator(self, data: List[int], ptr: int, size: int) -> None:
        if size >= 4:
            self.global_indicator_1 = util.to_long_uint(data, ptr)
            size -= 4
            ptr += 4
        if size >= 4:
            self.global_indicator_2 = util.to_long_uint(data, ptr)
            size -= 4
            ptr += 4
        if size >= 1:
        ...
        if size >= 0:
            i = 0
            while size >= 4:
                self.motion[i] = util.to_long_uint(data, ptr)
                i += 1
                ptr += 4
                size -= 4
...
class VL53L5CX:
    ...
    def _poll_for_answer(
        ...
        while first or (self.temp_buffer[pos] & mask) != expected_value:
            ...
            if timeout >= 200:  # 2s timeout
                ...
                raise VL53L5CXException(api.VL53L5CX_MCU_ERROR)
            else:
                timeout += 1
    ...
    def _send_offset_data(self, resolution: int) -> None:
        ...
        self.temp_buffer[:api.VL53L5CX_OFFSET_BUFFER_SIZE] = self.offset_data[:api.VL53L5CX_OFFSET_BUFFER_SIZE]
        ...
        if resolution == api.VL53L5CX_RESOLUTION_4X4:
            ...
            if DEBUG_LOW_LEVEL_LOGIC_SEND_OFFSET_DATA:
                print("_send_offset_data: signal_grid=[", end="")
            ...
            if DEBUG_LOW_LEVEL_LOGIC_SEND_OFFSET_DATA:
                print("_send_offset_data: range_grid=[", end="")
            ...
            for j in range(4):
                for i in range(4):
                    k = (i + 4 * j) * 4
                    l = ((2 * i) + (16 * j)) * 4
                    v = (util.to_long_uint(signal_grid, l + 0)
                         + util.to_long_uint(signal_grid, l + 1 * 4)
                         + util.to_long_uint(signal_grid, l + 8 * 4)
                         + util.to_long_uint(signal_grid, l + 9 * 4)) // 4
                    util.ulong_to_buffer(v, signal_grid, k)

                    k = (i + 4 * j) * 2
                    l = ((2 * i) + (16 * j)) * 2

                    v = (util.to_short_int(range_grid, l + 0)
                         + util.to_short_int(range_grid, l + 1 * 2)
                         + util.to_short_int(range_grid, l + 8 * 2)
                         + util.to_short_int(range_grid, l + 9 * 2))
                    if v < 0:
                        v = -(-v // 4)
                    else:
                        v = v // 4
                    util.short_to_buffer(v, range_grid, k)
        ...
        self.wr_multi(0x2e18, self.temp_buffer, api.VL53L5CX_OFFSET_BUFFER_SIZE)
    ...
    def _send_xtalk_data(self, resolution: int) -> None:
        ...
        self.temp_buffer[:api.VL53L5CX_XTALK_BUFFER_SIZE] = self.xtalk_data[0: api.VL53L5CX_XTALK_BUFFER_SIZE]
        ...
        self.wr_multi(0x2cf8, self.temp_buffer, api.VL53L5CX_XTALK_BUFFER_SIZE)
        self._poll_for_answer(4, 1, api.VL53L5CX_UI_CMD_STATUS, 0xff, 0x03)
    ...
    def init(self) -> None:
        ...
        if self.nb_target_per_zone != 1:
            ...
            self.dci_replace_data(self.temp_buffer, api.VL53L5CX_DCI_FW_NB_TARGET, 16, tmp, 1, 0x0C)

        self.dci_write_data(single_range, api.VL53L5CX_DCI_SINGLE_RANGE, len(single_range))
    ...
    def get_power_mode(self) -> int:
        ...
        if tmp == 0x4:
            p_power_mode = api.VL53L5CX_POWER_MODE_WAKEUP
        elif tmp == 0x2:
            p_power_mode = api.VL53L5CX_POWER_MODE_SLEEP
        else:
    ...
    def set_power_mode(self, power_mode: int) -> None:
        ...
        if power_mode != current_power_mode:
            if power_mode == api.VL53L5CX_POWER_MODE_WAKEUP:
                self.wr_byte(0x7FFF, 0x00)
                self.wr_byte(0x09, 0x04)
                self._poll_for_answer(1, 0, 0x06, 0x01, 1)
            elif power_mode == api.VL53L5CX_POWER_MODE_SLEEP:
                self.wr_byte(0x7FFF, 0x00)
    ...
    def start_ranging(self) -> None:
        ...
        if DEBUG_LOW_LEVEL_LOGIC_START_RANGING:
            print(f"api.VL53L5CX_start_ranging:    output_bh_enable[0]={output_bh_enable[0]:0{8}x}")
        ...
        for i in range(total_output_len):
            if (output[i] == 0) or ((output_bh_enable[i // DIVIDE_FACTOR] & (1 << (i % 32))) == 0):
                if DEBUG_LOW_LEVEL_LOGIC_START_RANGING:
                    print(f"api.VL53L5CX_start_ranging:    continue output[{i}]={output[i]:0{8}x}, output_bh_enable[{i // DIVIDE_FACTOR}]={output_bh_enable[i // DIVIDE_FACTOR]:0{8}x}")
                continue

            bh_ptr_type = output[i] & 0x0f
            bh_ptr_idx = (output[i] >> 16) & 0xffff
            if DEBUG_LOW_LEVEL_LOGIC_START_RANGING:
                print(f"api.VL53L5CX_start_ranging:    output[{i}]={output[i]:0{8}x}, bh_ptr_type={bh_ptr_type:0{4}x} bh_ptr_idx={bh_ptr_idx:0{8}x}")
            ...
            if 0x1 <= bh_ptr_type < 0x0d:
                ...
                if DEBUG_LOW_LEVEL_LOGIC_START_RANGING:
                    print(f"api.VL53L5CX_start_ranging:    output[{i}]={output[i]:0{8}x}, data_read_size={self.data_read_size}")
            else:
                bh_ptr_size = (output[i] >> 4) & 0xfff
                self.data_read_size += bh_ptr_size
            self.data_read_size += 4
            if DEBUG_LOW_LEVEL_LOGIC_START_RANGING:
                print(f"api.VL53L5CX_start_ranging:  data_read_size={self.data_read_size}")

        self.data_read_size += 20
        if DEBUG_LOW_LEVEL_LOGIC_START_RANGING:
            print(f"api.VL53L5CX_start_ranging:  final data_read_size={self.data_read_size}")

        output_bytes = util.long_array_to_bytes(output)
        self.dci_write_data(output_bytes, api.VL53L5CX_DCI_OUTPUT_LIST, len(output_bytes))

        header_config[0] = self.data_read_size
        header_config[1] = total_output_len + 1

        if DEBUG_LOW_LEVEL_LOGIC_START_RANGING:
            print(f"api.VL53L5CX_start_ranging:  header_config={header_config[0]:0{8}x} {header_config[1]:0{8}x}")

        header_config_bytes = util.long_array_to_bytes(header_config)
        self.dci_write_data(header_config_bytes, api.VL53L5CX_DCI_OUTPUT_CONFIG, len(header_config_bytes))

        output_bh_enable_bytes = util.long_array_to_bytes(output_bh_enable)
        self.dci_write_data(output_bh_enable_bytes, api.VL53L5CX_DCI_OUTPUT_ENABLES, len(output_bh_enable_bytes))
        ...
        self.wr_multi(api.VL53L5CX_UI_CMD_END - (4 - 1), cmd, len(cmd))
        self._poll_for_answer(4, 1, api.VL53L5CX_UI_CMD_STATUS, 0xff, 0x03)
        ...
        #     print(f"api.VL53L5CX_start_ranging: size={size} != data_read_size={self.data_read_size}, temp_buffer={self.temp_buffer[:32]}")
        #     # raise VL53L5CXException(api.VL53L5CX_STATUS_ERROR)
    ...
    def check_data_ready(self) -> bool:
        ...
        if ((self.temp_buffer[0] != self.streamcount) and
        ...
        if DEBUG_LOW_LEVEL_LOGIC:
            print(f"api.VL53L5CX_check_data_ready: buf={self.temp_buffer[:4]}, streamcount={self.streamcount}")
        return False
    ...
    def get_ranging_data(self) -> VL53L5CXResultsData:
        ...
        if not self.use_raw_format:
            ...
            if not self.disable_ambient_per_spad:
                for i in range(api.VL53L5CX_RESOLUTION_8X8):
                    p_results.ambient_per_spad[i] /= 2048

            for i in range((api.VL53L5CX_RESOLUTION_8X8 * self.nb_target_per_zone)):
                if not self.disable_distance_mm:
    ...
    def get_resolution(self) -> int:
        self.dci_read_data(self.temp_buffer, api.VL53L5CX_DCI_ZONE_CONFIG, 8)
        return self.temp_buffer[0x00] * self.temp_buffer[0x01]

    def set_resolution(self, resolution: int) -> None:
        if resolution == api.VL53L5CX_RESOLUTION_4X4:
            self.dci_read_data(self.temp_buffer, api.VL53L5CX_DCI_DSS_CONFIG, 16)
            self.temp_buffer[0x04] = 64
            self.temp_buffer[0x06] = 64
            self.temp_buffer[0x09] = 4
            self.dci_write_data(self.temp_buffer, api.VL53L5CX_DCI_DSS_CONFIG, 16)
            ...
            self.dci_write_data(self.temp_buffer, api.VL53L5CX_DCI_ZONE_CONFIG, 8)
        elif resolution == api.VL53L5CX_RESOLUTION_8X8:
            self.dci_read_data(self.temp_buffer, api.VL53L5CX_DCI_DSS_CONFIG, 16)
            ...
            raise VL53L5CXException(api.VL53L5CX_STATUS_INVALID_PARAM)

        self._send_offset_data(resolution)
        self._send_xtalk_data(resolution)

    def get_ranging_frequency_hz(self) -> int:
        self.dci_read_data(self.temp_buffer, api.VL53L5CX_DCI_FREQ_HZ, 4)
        return self.temp_buffer[0x01]

    def set_ranging_frequency_hz(self, frequency_hz: int) -> None:
        frequency_hz_buf = [frequency_hz]
        self.dci_replace_data(self.temp_buffer, api.VL53L5CX_DCI_FREQ_HZ, 4, frequency_hz_buf, 1, 0x01)

    def get_integration_time_ms(self) -> int:
        self.dci_read_data(self.temp_buffer, api.VL53L5CX_DCI_INT_TIME, 20)
        return util.to_long_uint(self.temp_buffer, 0) // 1000
    ...
    def set_integration_time_ms(self, integration_time_ms: int) -> None:
        ...
        buf = [0, 0, 0, 0]
        util.ulong_to_buffer(integration, buf)

        self.dci_replace_data(self.temp_buffer, api.VL53L5CX_DCI_INT_TIME, 20, buf, 4, 0x00)

    def get_sharpener_percent(self) -> float:
        self.dci_read_data(self.temp_buffer, api.VL53L5CX_DCI_SHARPENER, 16)
    ...
    def set_sharpener_percent(self, sharpener_percent: int) -> None:
        ...
        self.dci_replace_data(self.temp_buffer, api.VL53L5CX_DCI_SHARPENER, 16, sharpener, 1, 0xD)

    def get_target_order(self) -> int:
        self.dci_read_data(self.temp_buffer, api.VL53L5CX_DCI_TARGET_ORDER, 4)
        return self.temp_buffer[0x0]

    def set_target_order(self, target_order: int) -> None:
        if target_order == api.VL53L5CX_TARGET_ORDER_CLOSEST or target_order == api.VL53L5CX_TARGET_ORDER_STRONGEST:
            target_order_buf = [target_order]
            self.dci_replace_data(self.temp_buffer, api.VL53L5CX_DCI_TARGET_ORDER, 4, target_order_buf, 1, 0x0)
        else:
            raise VL53L5CXException(api.VL53L5CX_STATUS_INVALID_PARAM)

    def get_ranging_mode(self) -> int:
        self.dci_read_data(self.temp_buffer, api.VL53L5CX_DCI_RANGING_MODE, 8)

        if self.temp_buffer[0x01] == 0x1:
            return api.VL53L5CX_RANGING_MODE_CONTINUOUS

        return api.VL53L5CX_RANGING_MODE_AUTONOMOUS

    def set_ranging_mode(self, ranging_mode: int) -> None:
        self.dci_read_data(self.temp_buffer, api.VL53L5CX_DCI_RANGING_MODE, 8)

        if ranging_mode == api.VL53L5CX_RANGING_MODE_CONTINUOUS:
            self.temp_buffer[0x01] = 0x1
            self.temp_buffer[0x03] = 0x3
            single_range = 0x00
        elif ranging_mode == api.VL53L5CX_RANGING_MODE_AUTONOMOUS:
        ...
        self.dci_write_data(self.temp_buffer, api.VL53L5CX_DCI_RANGING_MODE, 8)

        buf = [single_range, 0, 0, 0]
        self.dci_write_data(buf, api.VL53L5CX_DCI_SINGLE_RANGE, 4)
    ...
    def dci_read_data(self,
        ...
        if (data_size + 12) > self.api.VL53L5CX_TEMPORARY_BUFFER_SIZE:
            ...
            self.wr_multi(api.VL53L5CX_UI_CMD_END - 11, cmd, len(cmd))
            self._poll_for_answer(4, 1, api.VL53L5CX_UI_CMD_STATUS, 0xff, 0x03)
